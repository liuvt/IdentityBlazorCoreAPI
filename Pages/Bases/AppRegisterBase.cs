...
public class AppRegisterBase : ComponentBase
{
    ...
    private async Task RegisterHandler(AppRegisterDTO _models)
    {
        try
        {
            if (await this.authService.Register(_models))
            {
    ...
    protected AppRegisterDTO models = new AppRegisterDTO();
    ...
    protected async void OnValidSubmit(EditContext editContext)
    {
        _processing = true;
        //Do something
        await RegisterHandler(models);
        StateHasChanged();
    }

    //Clean models
    protected async Task CleanForm() => models = new AppRegisterDTO();
